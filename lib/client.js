...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function collectToolPaths(root) {
      ...
      // `innerText` keeps the rendered line breaks that separate one path from
      // the next; with `textContent` a list of paths ran together
      // (`…index.jsD:\…\client.js`) and the capture swallowed the next drive
      // letter, which also lost every path after the first.
      add(root.innerText !== undefined ? root.innerText : (root.textContent || ''))
      return found
    }
