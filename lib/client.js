...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function normalizeAbs(path) {
      ...
      // Work on the part after the drive. `"D:\\a\\b".split(/[\\/]+/)` keeps `D:`
      // as its first segment, so re-adding the drive afterwards produced
      // `D:\D:\a\b` — which is what "Copy path" was handing out.
      const drive = isWin ? /^([A-Za-z]:)/.exec(path) : null
      const body = drive === null ? path : path.slice(drive[1].length)
      ...
      if (drive !== null) joined = drive[1] + '\\' + joined
    ...
    function detectFileTarget(node, x, y) {
      ...
      // Explicitly presented files render as a different row than the produced
      // chips (`[data-presented-files-row]`), and their preview button carries
      // the resolved absolute path as its title. Without this branch the right
      // button had no menu at all on presented deliverables.
      const presented = node.closest('[data-presented-files-row] button[title]')
      if (presented !== null) {
        const title = presented.getAttribute('title')
        if (typeof title === 'string' && title !== '') return { path: title }
      }
      // Fall back to the row itself: right-clicking the padding around a chip
      // should not fall through to the browser's own menu just because the
      // pointer missed the button by a few pixels. The nearest button wins when
      // the row carries several files.
      const filesRow = node.closest('[data-produced-files-row], [data-presented-files-row]')
      if (filesRow !== null && typeof filesRow.querySelectorAll === 'function') {
        const buttons = filesRow.querySelectorAll('button[title]')
        if (buttons.length > 0) {
          let best = buttons[0]
          ...
          const title = best.getAttribute('title')
          if (typeof title === 'string' && title !== '') return { path: title }
        }
      }
      ...
      if (row !== null) {
        ...
        if (button !== null) {
          ...
          // A folded tool row shows only a short label (`Read client.js`) while
          // the full path hides in the collapsed body. Resolve that label
          // against the card's own paths, so right-clicking the label behaves
          // exactly like right-clicking the path itself.
          if (text !== '' && text !== 'Inspect') {
            const own = collectToolPaths(row)
            const nameMatch = /([^\\/:*?"<>|\s]+\.[A-Za-z0-9]{1,8})/.exec(text)
            const wanted = nameMatch === null ? '' : nameMatch[1].toLowerCase()
            const named = wanted === '' ? [] : own.filter((path) => basename(path).toLowerCase() === wanted)
            if (named.length === 1) return { path: named[0] }
            const absolute = own.filter((path) => /^(?:[A-Za-z]:[\\/]|\\\\)/.test(path))
            if (absolute.length === 1) return { path: absolute[0] }
          }
        }
      }
      // Generic chip fallback: several surfaces render file chips under their
      // own class names (`dsh-tt-chip` from dsh-turn-tools, for one) and none of
      // them sits inside the rows above. What they share is a `title` holding
      // the absolute path — that is the durable contract to hook onto, and
      // without it their right-click fell through to the browser's own menu.
      const titledButton = node.closest('button[title]')
      if (titledButton !== null) {
        const title = titledButton.getAttribute('title')
        const value = typeof title === 'string' ? title.trim() : ''
        // A bare file name counts too. Chips like `_fileMention_*` carry only
        // the name they were written with, and the shell resolves it the same
        // way for its own click handler — so the menu here can never reach a
        // different file than the built-in action already does, while it does
        // add every action the built-in has no entry for.
    ...
    /**
     * Every path-looking token inside one tool card, in document order.
     *
     * A tool call renders its file references in several shapes — `title`
     * attributes on chips, plain text in arguments and results — so the
     * collector reads both instead of trusting one selector. Duplicates are
     * dropped case-insensitively because the same file often appears twice
     * (once in the arguments, once in the result).
     */
    function collectToolPaths(root) {
      const found = []
      const seen = new Set()
      const add = (value, allowBareName) => {
        const raw = typeof value === 'string' ? value.trim() : ''
        if (raw === '') return
        // Bound the scan: one card can hold tens of kilobytes of output, and the
        // head of it is enough to find the paths the call referred to.
        const text = raw.length > 32768 ? raw.slice(0, 32768) : raw
        const matches = textPathMatches(text)
        const candidates = matches.map((entry) => entry.path)
        if (candidates.length === 0) {
          if (/^(?:[A-Za-z]:[\\/]|\\\\)/.test(text)) candidates.push(text)
          else if (allowBareName === true && /^[^\\/:*?"<>|\r\n]+\.[A-Za-z0-9]{1,8}$/.test(text)) candidates.push(text)
        }
      ...
      if (typeof root.querySelectorAll === 'function') {
        for (const el of root.querySelectorAll('[title]')) add(el.getAttribute('title'))
        // The built-in tool row renders its file reference as a `fileLink`
        // button whose text is the only place the path appears — it carries no
        // title attribute at all.
        for (const el of root.querySelectorAll('[class*="fileLink"]')) add(el.textContent, true)
      }
      // Walk leaf by leaf instead of reading the card as one string.
      // `textContent` on the whole card concatenates siblings, so a list of
      // paths ran together (`…index.jsD:\…\client.js`) and the capture swallowed
      // the next drive letter while losing every later path; `innerText` keeps
      // rendered line breaks but skips collapsed content — which is exactly
      // where a folded card keeps its full paths. Per-leaf `textContent` gives
      // both: separated captures and hidden content.
      const walk = (element) => {
        if (element.children === undefined || element.children === null || element.children.length === 0) {
          add(element.textContent, true)
          return
        }
    ...
    /**
     * Describe the tool card under the pointer: its position among the cards
     * currently rendered, its name, and the files it touched.
     *
     * @returns `null` when the pointer is not inside a card, and a card with no
     * path at all when the call mentioned no files — the caller decides whether
     * an empty list is worth a menu.
     */
    function detectToolTarget(node) {
      if (node === null || typeof node !== 'object' || typeof node.closest !== 'function') return null
    ...
    const NS = 'filemenu'
    /** How many paths one tool card lists before the remainder collapses. */
    const TOOL_PATH_LIMIT = 20
    const zh = {
      'menu.header': '文件操作',
      'menu.open': '打开',
      'menu.reveal': '打开所在目录',
      ...
      'menu.toolHeader': '工具 {index}',
      'menu.copyAllPaths': '复制全部路径',
      'menu.moreFiles': '还有 {count} 个…',
      'menu.noToolFiles': '该工具调用没有可识别的文件路径'
    }
    const en = {
      ...
      'menu.toolHeader': 'Tool {index}',
      'menu.copyAllPaths': 'Copy all paths',
      'menu.moreFiles': '{count} more…',
      'menu.noToolFiles': 'No readable file path in this tool call'
    }
...
`
    ...
    function FileContextMenuHost(props) {
      ...
      React.useEffect(() => {
        const onContextMenu = (event) => {
          ...
          // TEMP: report what the pointer actually landed on, hit or miss, so the
          // host side can be inspected without devtools.
          try {
            const el = event.target
            const attrs = []
            if (el !== null && el !== undefined && el.attributes) {
              for (const attr of el.attributes) attrs.push(attr.name + '=' + String(attr.value).slice(0, 80))
            }
            const near = (selector) => el !== null && el !== undefined && typeof el.closest === 'function' && el.closest(selector) !== null
            rpc('diag', {
              kind: 'contextmenu',
              hit: target === null ? null : target.kind,
              tag: el !== null && el !== undefined && el.tagName ? el.tagName : '',
              cls: el !== null && el !== undefined && typeof el.className === 'string' ? el.className.slice(0, 160) : '',
              attrs: attrs.slice(0, 12),
              inProducedRow: near('[data-produced-files-row]'),
              inPresentedRow: near('[data-presented-files-row]'),
              inToolCard: near('[data-tool]')
            }).catch(() => {})
          } catch (error) { /* diagnostics only */ }
          if (target === null) {
            // Anywhere else on a tool card offers that call's own file list. A
            // right-click directly on one of the card's path chips stays a file
            // target, because that is the more precise answer.
            const toolHit = detectToolTarget(event.target)
            if (toolHit !== null) {
              target = { kind: 'tool', index: toolHit.index, name: toolHit.name, paths: toolHit.paths }
            }
          }
          if (target === null) {
            // Plain text is the last resort: a card mentioning exactly one path
            // used to swallow the right-click before the tool menu could offer
            // the call's whole file list.
            const plain = plainTextPathAtPoint(event.target, event.clientX, event.clientY)
            if (plain !== null) target = { kind: 'file', path: plain }
          }
      ...
      const abs = isFile ? normalizeAbs(joinPath(cwd, target.path)) : isTool ? '' : target.path
      const rel = isFile ? relativeTo(cwd, target.path, abs) : isTool ? '' : target.path
      const headerName = isFile
        ? basename(target.path)
        : isTool
          ? t('menu.toolHeader', { index: target.index })
          : target.title
      const headerTitle = isFile
        ? abs
        : isTool
          ? (target.name === '' ? target.paths.join('\n') : target.name + '\n' + target.paths.join('\n'))
          : target.path
      ...
      if (target.kind === 'workspace') {
        ...
        // One entry per file the call touched; clicking opens it, and a
        // right-click on any entry raises the full file menu for that path.
        const shown = target.paths.slice(0, TOOL_PATH_LIMIT)
        items = shown.map((path, index) => ({
          id: 'toolPath' + index,
          label: basename(path),
          hint: path,
          action: () => rpc('open', { path, cwd, sessionId })
        }))
        if (target.paths.length === 0) {
          // The card is still worth a menu: the header names the call even when
          // the row never rendered a path we could read.
          items.push({ id: 'toolEmpty', label: t('menu.noToolFiles'), disabled: true })
        }
        if (target.paths.length > shown.length) {
          items.push({ id: 'toolMore', label: t('menu.moreFiles', { count: target.paths.length - shown.length }), disabled: true })
        }
        items.push(null)
        items.push({ id: 'copyAbs', label: t('menu.copyAllPaths'), action: () => copyText(target.paths.join('\r\n')) })
      } else {
        items = [
      ...
      return React.createElement(React.Fragment, null, [
        React.createElement('div', {
          ...
          React.createElement('div', { key: 'header', className: 'fm-header', title: headerTitle }, headerName),
          items.map((item) => {
            ...
            return React.createElement('button', {
              ...
              className: 'fm-item',
              disabled: item.disabled === true,
              title: item.hint === undefined ? undefined : item.hint,
              // Right-clicking a listed path hands it to the ordinary file menu,
              // so every action that exists for a path also exists for a path
              // discovered inside a tool call.
              onContextMenu: item.hint === undefined ? undefined : (event) => {
                event.preventDefault()
                event.stopPropagation()
                setSub(null)
                setError(null)
                setDialog(null)
                setMenu({ x: event.clientX, y: event.clientY, target: { kind: 'file', path: item.hint } })
              },
    ...
    function apply(ctx) {
      RUNTIME.ctx = ctx
      // TEMP diagnostics: report what the shell actually rendered so the host
      // side can be inspected without devtools. Remove once the tool menu is
        ...
        const report = () => {
          try {
            rpc('diag', {
              ...
              rowButtons: document.querySelectorAll('[data-produced-files-row] button[title], [data-presented-files-row] button[title]').length,
              assistantSteps: document.querySelectorAll('[data-chat-flow-kind]').length
            }).catch(() => {})
          } catch (error) { /* diagnostics never break the plugin */ }
        }
        setTimeout(report, 1500)
        setTimeout(report, 6000)
        // Keep reporting: the host keeps the latest page snapshot beside the
        // latest right-click report, so either can be read at any time.
