...
export function apply(ctx) {
  ...
  // side can be inspected without devtools. Remove with the client half.
  let lastDiag = null
  ...
  const actions = {
    /** TEMP diagnostics channel (pairs with `lastDiag`). */
    diag: async (args) => {
      lastDiag = args === undefined || args === null ? null : args
      return { ok: true }
    },
    diagRead: async () => ({ ok: true, diag: lastDiag }),
