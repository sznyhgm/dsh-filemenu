...
/**
 * How long a launcher may keep running before it counts as launched.
 *
 * A desktop launcher often hands the request to an already-running instance and
 * exits — `Code.exe <file>` settles in ~2.4 s on this machine — so the watch
 * window has to outlast that handshake. Anything still running when the window
 * closes is a live application and keeps running.
 */
...
export function apply(ctx) {
  // TEMP diagnostics: the browser half posts what it actually hit here so this
  // side can be inspected without devtools. Page snapshots and right-click
  // reports are kept separately so neither overwrites the other.
  let lastPageDiag = null
  let lastContextDiag = null
  ...
  /**
   * Start one managed launcher and report how it ended.
   *
   * Fire-and-forget is wrong for an editor: a launcher that fails inside the
   * watch window used to come back as `ok:true` and leave the user staring at
   * nothing. Here a non-zero exit is a failure, an exit inside the window is a
   * completed handoff, and a process still running when the window closes has
   * launched and keeps running.
   */
  const spawnAndReport = async (argv, cwd) => {
    const subprocess = subprocessService()
    if (subprocess === undefined || typeof subprocess.spawn !== 'function') {
      return { ok: false, error: 'subprocess service unavailable' }
    }
    ...
    if (handle === undefined || handle.done === undefined) return { ok: true }
    const settled = await Promise.race([
      handle.done.then(
        (value) => ({ kind: 'exited', value }),
        (error) => ({ kind: 'failed', error: error instanceof Error ? error.message : String(error) })
      ),
      new Promise((resolve) => { setTimeout(() => resolve({ kind: 'running' }), LAUNCH_WATCH_MS) })
    ])
    if (settled.kind === 'failed') return { ok: false, error: settled.error }
    if (settled.kind === 'running') return { ok: true, running: true, exitCode: null }
    const exitCode = settled.value === undefined || settled.value === null ? null : settled.value.exitCode
    if (typeof exitCode === 'number' && exitCode !== 0) {
      return { ok: false, error: 'the launcher exited with code ' + exitCode, exitCode }
    }
    return { ok: true, running: false, exitCode: typeof exitCode === 'number' ? exitCode : null }
  }
  ...
  const actions = {
    /** TEMP diagnostics channel (pairs with the two `last*Diag` slots). */
    diag: async (args) => {
      const value = args === undefined || args === null ? null : args
      if (value !== null && value.kind === 'page') lastPageDiag = value
      else lastContextDiag = value
      return { ok: true }
    },
    diagRead: async () => ({ ok: true, page: lastPageDiag, context: lastContextDiag }),
    ...
    openWith: async (args) => {
      ...
      const launched = await spawnAndReport([editor.command, abs], dirnameOf(abs))
      if (!launched.ok) return { ok: false, error: launched.error, exitCode: launched.exitCode === undefined ? null : launched.exitCode }
      return {
        ok: true,
        path: abs,
        editor: editor.id,
        running: launched.running === true,
        exitCode: launched.exitCode === undefined ? null : launched.exitCode
      }
    },
